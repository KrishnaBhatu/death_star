...
BOT_NAME = ns[1:len(ns)-1]

ODOM_TOPIC = '/robot_' + BOT_NAME + "/base_pose_ground_truth"
LASER_TOPIC = '/robot_' + BOT_NAME + "/base_scan"
PATH_TOPIC = '/robot_' + BOT_NAME + "/nav_path"
CMD_VEL = '/robot_' + BOT_NAME + "/cmd_vel"
...
class Config():
    ...
    def smartPlanRequest(self, start_x, start_y, goal_x, goal_y):
        rospy.wait_for_service('gen_path')
        print("Service available for ", BOT_NAME)
        try:
            path = rospy.ServiceProxy('gen_path', smartPlan)
            response = path(start_x, start_y, goal_x, goal_y)
            return response
        except rospy.ServiceException, e:
            print "Service call failed: %s"%e
            ...
            if goalCoord.task_available:
                ...
                if path_death_star:
                    for p in path_death_star.Path.poses:
                        config.path.append([p.pose.position.x, p.pose.position.y])
                    config.goalX = config.path[0][0]
                    config.goalY = config.path[0][1]
                else:
                    continue;    
